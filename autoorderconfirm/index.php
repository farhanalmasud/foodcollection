...
<?php
			...
			function getuserdata($username){
			     global $conn;
			     $queryyu = "SELECT * FROM `users` WHERE `id`='$username'";
...
curl_setopt_array($curl, array(
  ...
  CURLOPT_POSTFIELDS => 'key=MksdjsjdsnjcdccudcdJsw83&phone='.$phone.'&type=orderconfirmcustomer&orderid='. $rowwid,
...
$sql = "SELECT * FROM `orders` WHERE `order_status` = 'accepted' AND `call_status` IN (0,1,2) ORDER BY FIELD(`call_status`, 0,1,2) LIMIT 1";
...
if ($result && mysqli_num_rows($result) > 0) {
    while ($row = mysqli_fetch_assoc($result)) {
        ...
        $number = str_replace("+88", "", $userdata['phone']);
        ...
        if($updatesarray['success'] === true){
            ...
            $sql6 = "UPDATE `orders` SET `call_status`=1 WHERE `id` =$rowwid";
            mysqli_query($conn, $sql6);
        }else{
