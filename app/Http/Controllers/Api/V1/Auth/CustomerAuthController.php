...
class CustomerAuthController extends Controller
{
    ...
    {
        $ip = $request->ip();
        $rateLimitKey = 'customer_registration_ip_' . $ip;
        $maxAttempts = 5;
        ...
        if (RateLimiter::tooManyAttempts($rateLimitKey, $maxAttempts)) {
            ...
            $timeText = $hours > 1 ? $hours . ' ' . translate('messages.hours') : ceil($seconds / 60) . ' ' . translate('messages.minutes');
            $errors = [];
            $errors[] = [
                'code' => 'rate_limit',
                'message' => translate('messages.Too_many_attemps') . '. ' . translate('messages.please_try_again_after_') . $timeText
            ];
        ...
        $validator = Validator::make($request->all(), [
            ...
            'name.min' => translate('The name must be at least 2 characters.'),
            'name.regex' => translate('The name field must contain only valid alphabetic characters.'),
        ]);
        ...
        if (empty($firstName) || is_numeric($firstName) || !preg_match('/[\p{L}]/u', $firstName)) {
            $errors = [];
            $errors[] = ['code' => 'name', 'message' => translate('The name field must contain valid letters.')];
            return response()->json(['errors' => $errors], 403);
        }
