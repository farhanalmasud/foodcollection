...
class CustomerAuthProvider implements CustomerAuthProviderContract
{
    ...
    {
        $ip = request()->ip();
        $rateLimitKey = 'storefront_registration_ip_' . $ip;
        $maxAttempts = 5;
        ...
        if (RateLimiter::tooManyAttempts($rateLimitKey, $maxAttempts)) {
            ...
            $timeText = $hours > 1 ? $hours . ' ' . __('messages.hours') : ceil($seconds / 60) . ' ' . __('messages.minutes');
            throw ValidationException::withMessages([
                '_form' => __('messages.Too_many_attemps') . '. ' . __('messages.please_try_again_after_') . $timeText,
            ]);
        }
        ...
        if ($name === '') {
            ...
            $errors['name'] = [__('The name must be at least 2 characters.')];
        } elseif (!\preg_match('/^[\p{L}\s\.\'\-]+$/u', $name) || !\preg_match('/[\p{L}]/u', $name)) {
            $errors['name'] = [__('The name field must contain only valid alphabetic characters.')];
        }
        ...
        if (empty($firstName) || \is_numeric($firstName) || !\preg_match('/[\p{L}]/u', $firstName)) {
            throw ValidationException::withMessages([
                'name' => [__('The name field must contain valid letters.')],
            ]);
        }
